...
const MovieCollections = () => {
  const localCollections = JSON.parse(localStorage.getItem("movie-collections"));
  ...
  useEffect(() => {
    if (data ) {
      ...
      localStorage.setItem("movie-collections", JSON.stringify(result, null, 2));
    }
  }, [data]);
  ...
  return (
    ...
    <Container fluid className="main-content-container px-4">
      <Row className="page-header py-4">
        <PageTitle title="Movie Collections" subtitle="My Movie Collections" className="text-sm-left mb-3" />    
      </Row>
