...
# graphql query
begin_gql = """mutation{ accountConfirmEmail{"""
input_vars = ""
end_gql = """response{ success code message version 
    } pageInfo{ page_info_count
    } result{ account_info_verified_email account_info_registration_status account_info_status
    }}}
"""
graphql_info = gql(begin_gql + input_vars + end_gql)
general = lib.gen.compose_decos([pytest.mark.account_confirm_email_mutation, pytest.mark.account])
...
def test_account_confirm_email_mutation_response(benchmark):
    # clear db tables and reset
    lib.gen.reset_database()
    redis_db = lib.gen.db.get_engine("redisdb_movie", "redis")
    redis_db.flushdb()
    ...
    assert result["data"][QUERY_NAME]["result"] == [{
        "account_info_verified_email": True,
