...
QUERY_NAME = "movieDelete"

# graphql query
begin_gql = """mutation{ movieDelete( movie_fav_info_id: """
input_vars = f""" "1" """
...
general = lib.gen.compose_decos([pytest.mark.movie_delete_mutation, pytest.mark.movie])
...
def test_movie_delete_mutation_response(benchmark):
    # clear db tables and reset
    lib.gen.reset_database()
    ...
    input_vars = f""" "{movie['movie_fav_info_id']}" """
    graphql_info = gql(begin_gql + input_vars + end_gql)
