...
class General:
    ...
    def hash_password(self, password: str) -> str:
        return hashpw(password.encode('utf8'), gensalt(int(environ['APP_DEFAULT_GEN_SALT_VALUE'])))
    ...
    def token_gen(self, id, service: str, hr: int=336, email: bool=False, reg: str="NOTCOMPLETE", status: str="ACTIVE") -> str:
        ...
        secret = environ['APP_DEFAULT_EMAIL_KEY'] if email else environ['APP_DEFAULT_ACCESS_KEY']
        payload = {
            'user_id': id,
    ...
    def send_to_sendgrid(self, msg: dict, template: str) -> bool:
        if environ['APP_DEFAULT_ENV'] in ["dev", "local", "prod"]:
            # send to sendgrid
            SENDGRID_API_KEY = environ['SENDGRID_API_KEY']
            APP_ADDRESS = environ['APP_ADDRESS']
            send_to_email = msg["email"] if environ['APP_DEFAULT_EMAIL'] == 'prod' else str(environ['APP_DEFAULT_EMAIL'])
            message = Mail(
            ...
            if template == "VerifyEmail":
                ...
                if environ['APP_DEFAULT_ENV'] == 'prod':
