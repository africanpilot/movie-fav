...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "registration_status",
            sa.Enum(AccountRegistrationEnum),
            nullable=False,
            server_default=AccountRegistrationEnum.NOT_COMPLETE.name,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "business_type",
            sa.Enum(AccountBusinessTypeEnum),
            nullable=False,
            server_default=AccountBusinessTypeEnum.LLC.name,
        ),
        sa.Column("dba", sa.VARCHAR(255)),
        sa.Column("phone_number", sa.VARCHAR(12)),
        sa.Column(
            "classification",
            sa.Enum(AccountClassificationEnum),
            nullable=False,
            server_default=AccountClassificationEnum.OTHER.name,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "account_company_id",
            sa.INTEGER,
            sa.ForeignKey("account.account_company.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("ein", sa.VARCHAR(50), nullable=False, unique=True),
        sa.Column("name", sa.VARCHAR(255), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "account_info_id", sa.INTEGER, sa.ForeignKey("account.account_info.id", ondelete="CASCADE"), nullable=False
        ),
