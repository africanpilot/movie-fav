...
class AccountSagaStateCreate(LinkResponse):
  ...
  def account_saga_state_create(self, db: Session, payload: dict, commit: bool = True) -> AccountSagaState:
    ...
    sql_query = (insert(AccountSagaState)
      .values(
      **createInput.dict(exclude_unset=True)
    )).on_conflict_do_update(constraint='account_saga_state_account_info_id_key', set_=dict(**createInput.dict(exclude_unset=True, exclude={"account_info_id"}), updated=datetime.now()))

    if commit:
      db.exec(sql_query)
      db.commit()
      return db.exec(select(AccountSagaState).where(AccountSagaState.account_info_id == payload['account_info_id'])).one()
