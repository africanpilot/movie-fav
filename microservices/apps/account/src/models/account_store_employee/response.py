...
from account.src.models.account_store_employee.base import AccountStoreEmployee, AccountStoreEmployeeBase, AccountStoreEmployeePageInfoInput, AccountStoreEmployeeFilterInput
from sqlmodel import Session
...
class AccountStoreEmployeeResponses(LinkRequest):
  ...
  def account_store_employee_response(
    ...
    account_store_employee = self.convert_to_db_cols_with_attr(resultObject=AccountStoreEmployee, info=info, query_context=query_context, root_node=baseRootNode)
    ...
    return self.success_response(result=result, pageInfo=page_info, nullPass=True, resultObject=AccountStoreEmployeeResponse)
