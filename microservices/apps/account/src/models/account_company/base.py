...
class AccountCompanyBase(SQLModel):
    ...
    business_type: Optional[AccountBusinessTypeEnum] = Field(
        default=AccountBusinessTypeEnum.LLC, sa_column=Column(Enum(AccountBusinessTypeEnum))
    )
    dba: Optional[str] = Field(default=None, max_length=255)
    phone_number: Optional[str] = Field(default=None, max_length=12)
    classification: Optional[AccountClassificationEnum] = Field(
        default=AccountClassificationEnum.OTHER, sa_column=Column(Enum(AccountClassificationEnum))
    )
