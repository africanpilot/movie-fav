...
class AccountCompanyResponses(LinkRequest):
    ...
    def account_company_response_array(
        self,
        accountCompany: Optional[list[AccountCompany]],
        accountStore: Optional[list[AccountStore]],
        accountStoreEmployee: Optional[list[AccountStoreEmployee]],
    ):
        ...
        return select(*accountCompany, account_store, func.count().over().label("page_info_count"))
    ...
    def account_company_response(
        ...
        query = self.account_company_response_array(
            self.convert_to_db_cols_with_attr(
                resultObject=AccountCompany, info=info, query_context=query_context, root_node=baseRootNode
            ),
            self.convert_to_db_cols_with_attr(
                resultObject=AccountStore, info=info, query_context=query_context, root_node="account_store"
            ),
            self.convert_to_db_cols_with_attr(
                resultObject=AccountStoreEmployee,
                info=info,
                query_context=query_context,
                root_node="account_store_employee",
            ),
        )
        ...
        return self.success_response(
            ...
            pageInfo=page_info,
            nullPass=True,
            resultObject=AccountCompanyResponse,
            resultBase=AccountCompanyBaseResponse,
        )
