...
class AccountCreateMutation(
    ...
    def load_defs(self):
        ...
        def resolve_account_create(
            _, info: GraphQLResolveInfo, createInput: AccountInfoCreateInput
        ) -> AccountInfoResponse:

            service_name = self.check_service_authorized(info)

            createInput = AccountInfoCreateInput(
                **createInput, registration_status=AccountRegistrationEnum.NOT_COMPLETE
            )
            ...
            with self.get_session("psqldb_account") as db:
                ...
                if (
                    ...
                    acount_users_not_registered = list(
                        filter(
                            lambda x: x.registration_status
                            not in [AccountRegistrationEnum.APPROVED, AccountRegistrationEnum.COMPLETE],
                            acount_users,
                        )
                    )
