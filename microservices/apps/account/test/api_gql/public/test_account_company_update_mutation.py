...
def test_account_company_update_mutation(benchmark, test_database, flush_redis_db, private_schema, create_account):
    ...
    variables = dict(
        updateInput=dict(
            name="test company name update", cover_image="test cover_image", logo="test", profile_thumbnail="test"
        )
    )
