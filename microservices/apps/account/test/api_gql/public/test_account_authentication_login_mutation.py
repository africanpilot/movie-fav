...
GENERAL_PYTEST_MARK = LinkGeneral().compose_decos(
    [pytest.mark.account_authentication_login_mutation, pytest.mark.account]
)


@GENERAL_PYTEST_MARK
@pytest.mark.account_bench
def test_account_authentication_login_mutation(
    benchmark, test_database, private_schema, create_auth_info, create_account, link_account_lib: GeneralAccountLib
):
    ...
    variables = dict(accountLoginInput=dict(login=auth_1["rand_login"], password=auth_1["rand_password"]))
