...
export default function DetailsPage({ params }: { params: { slug: string } }) {
    ...
    const orderBy = OrderByEnum.Desc;
	  const { movieDetailsData, isSaving: loading } = useMovie({ first, sortBy, orderBy, pageNumber: 1 }, {id: [parseInt(params.slug)]});
    
    return (
      <MainLayout>
        <div>
          <section id={`movies-slug-${params.slug}`}><DetailsBanner movieDetails={movieDetailsData!}/></section>
        </div>
      </MainLayout>
    );
}
