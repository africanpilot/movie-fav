...
export default function SeasonDetailsPage({ params }: { params: { slug: string, id: string } }) {
  const first = 20;
  ...
  const episodes = showsDetailsData?.shows_season?.filter((x) => x?.season == parseInt(params.id))?.[0]?.shows_episode;
