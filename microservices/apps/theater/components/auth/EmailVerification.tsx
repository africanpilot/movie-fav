...
const EmailVerification = () => {
  ...
  const { resendEmail, isSaving } = useAccount();
    ...
    <div className="pt-28 text-center xl:text-left">
      <div className="p-6 md:mx-auto">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600 my-2">{`Please take a moment and validate your email to confirm your account. We sent a verification email to ${email}`}</p>
          ...
          <div className="py-10 text-center">
            <Link
              href={MapData.signinPage}
              target="_top"
              className="bg-green-400 hover:bg-green-300 text-gray-700 py-2 px-6 rounded-full transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg"
            >{`Login`}</Link>
          </div>
        </div>
      </div>
