...
const Home: React.FC<SumexusSettingsInterfaceProps> = ({ props }) => {
  ...
  const { moviePopular, isSaving: movieLoading } = useMovie({
    first: 8,
    sortBy: [MovieInfoSortByEnum.PopularId],
    orderBy,
  });
  const { showsPopular, isSaving: showsLoading } = useShows({
    first: 8,
    sortBy: [ShowsInfoSortByEnum.PopularId],
    orderBy,
  });
      ...
      {isLoading ? (
        ...
        <div className="relative pl-4 pb-24 lg:space-y-24">
          ...
          <div className="md:space-y-24 pt-10">
            <Row
              movies={moviePopular?.result as MovieInfo[]}
              title="Trending Movies"
              isMain={true}
              category={"movies"}
            />
            <Row
              movies={showsPopular?.result as ShowsInfo[]}
