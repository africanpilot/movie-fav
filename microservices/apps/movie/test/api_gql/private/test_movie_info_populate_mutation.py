...
def test_movie_info_popular_mutation(benchmark, test_database: Session, flush_redis_db, create_account, private_schema):
    ...
    # monkey patch
    with (
        patch("link_domain.imdb_helper.base.ImdbHelper.get_charts_imdbs", return_value=["0133093"]),
        patch(
            "link_domain.imdb_helper.base.ImdbHelper.get_popular_movies_ids",
            new_callable=lambda: property(lambda self: ["0133093"]),
        ),
    ):
