...
QUERY_NAME = "movieInfoUpdate"

qgl_query = gql(
    """
mutation movieInfoUpdate ($pageInfo: MovieInfoPageInfoInput, $updateFilterInput: MovieInfoUpdateFilterInput!) {
  movieInfoUpdate(pageInfo: $pageInfo, updateFilterInput: $updateFilterInput) {
    ...MovieInfoResponse
  }
}
"""
...
GENERAL_PYTEST_MARK = LinkGeneral().compose_decos([pytest.mark.movie_info_update_mutation, pytest.mark.movie])


@GENERAL_PYTEST_MARK
@pytest.mark.movie_bench
def test_movie_info_update_mutation(
    benchmark, test_database: Session, flush_redis_db, create_account, private_schema, create_movie_info
):
    flush_redis_db()

    _, auth_1 = create_account(test_database)

    variables = dict(pageInfo=dict(first=1), updateFilterInput=dict(imdb_ids=["0133093"]))
    movie_1 = create_movie_info(test_database)[0]
    assert movie_1.imdb_id == "0133093"
    ...
    assert movie_1.payload["cover"].startswith(
    ...
    assert movie_1.payload["full_cover"].startswith(
        "https://m.media-amazon.com/images/M/MV5BN2NmN2VhMTQtMDNiOS00NDlhLTliMjgtODE2ZTY0ODQyNDRhXkEyXkFqcGc@._V1_.jpg"
    )
    assert movie_1.payload["release_date"] is None
