...
class OptimizedMovieSagaStateUpdate(AbstractSagaStateRepository, LinkRedis):
    """
    ...
    when processing thousands of movie saga states.
    """
    ...
    def _execute_pending_updates(self):
        ...
        if self.use_thread_local_pool:
            session = ConnectionPoolManager.get_shared_session(self, "psqldb_movie")
        else:
            session = self.get_session("psqldb_movie")
    ...
    def _get_session(self):
        ...
        if self.use_thread_local_pool:
            return ConnectionPoolManager.get_shared_session(self, "psqldb_movie")
        else:
            return self.get_session("psqldb_movie")
