...
class MovieSagaStateCreate(LinkResponse):
    ...
    def movie_saga_state_create(
        self, db: Session, createInput: list[MovieSagaStateCreateInput], commit: bool = True
    ) -> Union[list[MovieSagaState], list[Insert]]:
        sql_query = []

        for movie in createInput:
            sql_query.append(
                insert(MovieSagaState)
                .values(**movie.dict(exclude_unset=True))
                .on_conflict_do_update(
                    constraint="movie_saga_state_movie_info_imdb_id_key",
                    set_=dict(**movie.dict(exclude_unset=True), updated=datetime.now()),
                )
            )
