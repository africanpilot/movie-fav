...
from link_models.messaging import MOVIE_COMMANDS_QUEUE, movie_import_message as message


class MovieImportSaga(StatefulSaga):
  def __init__(self, *args, **kwargs):
    ...
    self.steps = [
      AsyncStep(
        name='movie_import',
        action=self.movie_import,

        base_task_name=message.TASK_NAME,
        queue=MOVIE_COMMANDS_QUEUE,

        on_success=self.movie_import_on_success,
        on_failure=self.movie_import_on_failure
      ),
    ]
  
  def movie_import(self, current_step: AsyncStep):
    self.gen.log.info(f'Importing movie for movie_info_imdb_id {self.saga_state.movie_info_imdb_id} ...')

    message_id = self.send_message_to_other_service(
      current_step,
      message.MovieImportMessage(
