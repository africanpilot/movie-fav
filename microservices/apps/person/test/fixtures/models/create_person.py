...
def create_person_saga_state(person_lib: PersonLib) -> PersonSagaState:
  def create(db) -> PersonSagaState:
    return person_lib.person_saga_state_create.person_saga_state_create(db,
...
def create_person_info(person_lib: PersonLib) -> PersonInfo:
  def create(db) -> PersonInfo:
    return person_lib.person_info_create.person_info_create_imdb(db,
      [PersonInfoCreateInput(**PERSON_PAYLOAD)]
    )
  return create
