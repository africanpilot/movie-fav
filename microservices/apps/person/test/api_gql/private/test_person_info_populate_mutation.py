...
def test_person_info_populate_mutation(benchmark, test_database: Session, flush_redis_db, create_account, private_schema):
  flush_redis_db()
  ...
  with patch('link_api.grpc.movie.MovieGrpcClient.get_remaining_movie_cast_query', return_value={"message": {"cast_ids": ["0000206"]}}), \
      patch('link_api.grpc.shows.ShowsGrpcClient.get_remaining_shows_cast_query', return_value={"message": {"cast_ids": ["0000206"]}}):
  ...
  assert person_1.payload['head_shot'] == 'https://m.media-amazon.com/images/M/MV5BNDEzOTdhNDUtY2EyMy00YTNmLWE5MjItZmRjMmQzYTRlMGRkXkEyXkFqcGc@.jpg'
