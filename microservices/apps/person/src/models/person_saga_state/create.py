...
class PersonSagaStateCreate(LinkResponse):
  ...
  def person_saga_state_create(self, db: Session, createInput: list[PersonSagaStateCreateInput], commit: bool = True) -> Union[list[PersonSagaState], list[Insert]]:
    sql_query = []

    for person in createInput:
      sql_query.append(
        insert(PersonSagaState).values(**person.dict(exclude_unset=True))
        .on_conflict_do_update(constraint='person_saga_state_person_info_imdb_id_key', set_=dict(**person.dict(exclude_unset=True), updated=datetime.now()))
      )
