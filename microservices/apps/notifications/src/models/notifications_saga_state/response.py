...
class NotificationsSagaStateResponses(LinkRequest):
    ...
    def notifications_saga_state_response(
        ...
        if info:
            ...
            notifications_saga_state = self.convert_to_db_cols_with_attr(
                resultObject=NotificationsSagaState, info=info, query_context=query_context, root_node=baseRootNode
            )
        ...
        return self.success_response(
            result=result, pageInfo=page_info, nullPass=True, resultObject=NotificationsSagaStateResponse
        )
