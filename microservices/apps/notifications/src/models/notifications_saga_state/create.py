...
class NotificationsSagaStateCreate:
  ...
  def notifications_saga_state_create(self, db: Session, createInput: list[NotificationsSagaStateCreateInput], commit: bool = True) -> Union[list[NotificationsSagaState], list[Insert]]:
    sql_query = []

    for notification in createInput:
      sql_query.append(
        insert(NotificationsSagaState)
        .values(
          **notification.dict(exclude_unset=True, exclude={"body", "modified_body"}),
          ...
          modified_body=notification.modified_body.dict(exclude_unset=True) if notification.modified_body else None
        )
      )
    ...
    if commit:
      ...
      return db.exec(select(NotificationsSagaState).where(text(f"notifications_saga_state.body->>'email' IN({quoted_emails})"))).all()
