...
GENERAL_PYTEST_MARK = LinkGeneral().compose_decos(
    [pytest.mark.notifications_saga_state_query, pytest.mark.notifications]
)


@GENERAL_PYTEST_MARK
@pytest.mark.notifications_bench
def test_notifications_saga_state_query(
    benchmark, test_database: Session, flush_redis_db, create_account, create_notifications_saga_state, private_schema
):
    flush_redis_db()

    _, auth_1 = create_account(test_database)

    notification_1 = create_notifications_saga_state(
        test_database,
        {
    ...
    variables = dict(pageInfo=dict(first=1), filterInput=dict(id=[notification_1.id]))
