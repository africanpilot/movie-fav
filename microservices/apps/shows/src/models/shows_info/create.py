...
from shows.src.models.shows_season import ShowsSeason, ShowsSeasonCreateInput, ShowsSeasonRead
        ...
        for show in createInput:
            ...
            sql_query.append(
                (
                    ...
                    constraint="shows_info_imdb_id_key",
                    set_=dict(**show.model_dump(exclude_unset=True, exclude={"shows_season"}), updated=datetime.now()),
                )
            )
            ...
            for season in show.shows_season or []:
                ...
                sql_query.append(
                    (
                        ...
                        constraint="shows_season_imdb_id_key",
                        set_=dict(
                            **season.model_dump(exclude_unset=True, exclude={"shows_episode"}), updated=datetime.now()
                        ),
                    )
                )
                ...
                for episode in season.shows_episode or []:
                    ...
                    sql_query.append(
                        (
                            ...
                            constraint="shows_episode_imdb_id_key",
                            set_=dict(**episode.model_dump(exclude_unset=True), updated=datetime.now()),
                        )
                    )
        if commit:
            for r in sql_query:
