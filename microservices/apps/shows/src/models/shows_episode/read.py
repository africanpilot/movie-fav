...
class ShowsEpisodeRead(LinkResponse):
    ...
    def get_shows_episode(
        self, db: Connection, shows_info_id: int, shows_season_id: int, shows_episode_id: int
    ) -> ShowsEpisode:
        ...
        sql_query = self.join_tables(
            ...
            [
                dict(
                    ...
                    target=ShowsEpisode,
                    isouter=False,
                    onclause=(ShowsInfo.id == ShowsEpisode.shows_info_id),
                ),
            ],
        )
