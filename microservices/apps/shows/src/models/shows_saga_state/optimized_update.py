...
class OptimizedShowsSagaStateUpdate(AbstractSagaStateRepository, LinkRedis):
    """
    ...
    when processing thousands of shows saga states.
    """
    ...
    def _execute_pending_updates(self):
        ...
        if self.use_thread_local_pool:
            session = ConnectionPoolManager.get_shared_session(self, "psqldb_shows")
        else:
            session = self.get_session("psqldb_shows")
    ...
    def _get_session(self):
        ...
        if self.use_thread_local_pool:
            return ConnectionPoolManager.get_shared_session(self, "psqldb_shows")
        else:
            return self.get_session("psqldb_shows")
