...
class ShowsInfoUpdateMutation(GraphQLModel, ShowsLib):
    ...
    def load_defs(self):
        ...
        def resolve_shows_info_update(
            _, info: GraphQLResolveInfo, updateFilterInput: ShowsUpdateFilterInput, pageInfo: ShowsInfoPageInfoInput = None
        ) -> ShowsInfoResponse:
            ...
            with self.get_session("psqldb_shows") as db:
                all_saga = self.shows_saga_state_create.shows_saga_state_create(db, [ShowsSagaStateCreateInput(shows_info_imdb_id=imdb) for imdb in updateFilterInput.imdb_ids])
