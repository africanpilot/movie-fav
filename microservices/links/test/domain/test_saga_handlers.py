...
from link_lib.saga_framework.saga_handlers import saga_step_handler
from link_lib.saga_framework.utils import serialize_saga_error
...
def test_saga_step_handler_success():
    ...
    fake_celery_app.emulate_celery_task_launch(
        SAGA_STEP_NAME, saga_id=fake_saga_id, payload={"some": "saga step payload that orchestrator passed"}
    )
    ...
    fake_celery_app.send_task.assert_called_with(
        f"{SAGA_STEP_NAME}.response.success", args=[fake_saga_id, RESPONSE_PAYLOAD], queue=RESPONSE_QUEUE_NAME
    )
...
def test_saga_step_handler_failure():
    ...
    fake_celery_app.send_task.assert_called_with(
        ...
        args=[fake_saga_id, asdict(serialize_saga_error(exception_that_step_handler_raises))],
