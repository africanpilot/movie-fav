...
def test_join_tables(benchmark, link_response: GeneralLinkResponse):
    ...
    compile_sql.assert_compile(
        sql_query,
        "SELECT fake_info.email " "FROM fake_info JOIN fake_contact ON fake_info.id = fake_contact.fake_info_id",
        dialect=postgresql.dialect(),
    )
...
def test_query_filter(benchmark, link_response: GeneralLinkResponse):
    ...
    compile_sql.assert_compile(
        sql_query,
        "SELECT fake_info.email " "FROM fake_info WHERE fake_info.email = %(email_1)s",
        dialect=postgresql.dialect(),
...
def test_general_response_model(benchmark, link_response: GeneralLinkResponse):
    ...
    assert response == BaseResponse(response=GeneralResponse(), pageInfo=PageInfo(), result=[])
...
def test_update_general_response(benchmark, link_response: GeneralLinkResponse):
    ...
    assert response == BaseResponse(
        response=GeneralResponse(code=200, success=True, message="Success"), pageInfo=PageInfo(), result=[]
    )
...
def test_success_response(benchmark, link_response: GeneralLinkResponse):
    # test works
    response_1 = link_response.success_response(
        resultObject=BaseResponse, result=["test"], pageInfo=PageInfo(page_info_count=10)
    )
    ...
    # test nullPass pass
    response_2 = link_response.success_response(
        resultObject=BaseResponse, nullPass=True, result=[], pageInfo=PageInfo(page_info_count=10)
    )
    ...
    # run benchmark
    benchmark(
        link_response.success_response,
        resultObject=BaseResponse,
        result=["test"],
        pageInfo=PageInfo(page_info_count=10),
    )
