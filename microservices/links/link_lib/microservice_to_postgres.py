...
class DbConn:
    ...
    def get_engine(self, dbName: str, dbType: str = "postgresql") -> object:
        conn = {
            "db": {
                "postgresql": {
                    f"psqldb_{ms}": os.getenv(f"DB_POSTGRES_{ms.upper()}") for ms in self.general.enabled_microservices
                },
                "redis": {
                    f"redisdb_{ms}": os.getenv(f"DB_REDIS_{ms.upper()}") for ms in self.general.enabled_microservices
                },
            }
        }
