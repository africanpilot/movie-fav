...
from link_lib.saga_framework.base_saga import BaseStep
from link_lib.saga_framework.utils import failure_task_name, success_task_name
...
class StatefulSaga(AsyncSaga, abc.ABC):
    ...
    @classmethod
    def register_async_step_handlers(cls, saga_state_repository: AbstractSagaStateRepository, celery_app: Celery):
        # noinspection PyTypeChecker
        dummy_saga_instance = cls(None, None, None)

        for step in dummy_saga_instance.async_steps:
            cls.register_success_handler_for_step(saga_state_repository, celery_app, step)
            cls.register_failure_handler_for_step(saga_state_repository, celery_app, step)

    @classmethod
    def register_success_handler_for_step(
        cls, saga_state_repository: AbstractSagaStateRepository, celery_app: Celery, step: AsyncStep
    ):
        def on_success_handler(celery_task: Task, saga_id: int, payload: dict):
            saga = cls(saga_state_repository=saga_state_repository, celery_app=celery_app, saga_id=saga_id)
    ...
    @classmethod
    def register_failure_handler_for_step(
        cls, saga_state_repository: AbstractSagaStateRepository, celery_app: Celery, step: AsyncStep
    ):
