...
import asyncapi
from link_lib.saga_framework import SagaErrorPayload, failure_task_name, success_task_name
...
def message_to_channel(
    message: asyncapi.Message, response: asyncapi.Message = None, publish_made_first=False, description: str = None
) -> typing.Tuple[str, asyncapi.Channel]:
...
def asyncapi_message_for_success_response(
    base_task_name: str, title: str = None, summary: str = None, payload_dataclass: object = None
):
    return asyncapi.Message(
        name=success_task_name(base_task_name), title=title, summary=summary, payload=payload_dataclass
    )


def asyncapi_message_for_failure_response(
    base_task_name: str, title: str = None, summary: str = None, payload_dataclass: type = SagaErrorPayload
):
    return asyncapi.Message(
        name=failure_task_name(base_task_name), title=title, summary=summary, payload=payload_dataclass
    )
