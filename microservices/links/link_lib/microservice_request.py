...
from graphql import GraphQLResolveInfo, FragmentSpreadNode, FragmentDefinitionNode
from datetime import datetime, timedelta
from pydantic import BaseModel

from link_lib.microservice_to_redis import LinkRedis
from link_lib.microservice_response import LinkResponse
from link_models.enums import AccountRegistrationEnum, AccountStatusEnum, ServiceNameEnum, AccountRoleEnum
...
class LinkRequest(LinkRedis, LinkResponse):
  ...
  def check_service_authorized(self, info: GraphQLResolveInfo, services: list[ServiceNameEnum] = None) -> ServiceNameEnum:
    services = services or [
      ServiceNameEnum.MOVIEFAV,
      ServiceNameEnum.LABELLE,
      ServiceNameEnum.UVGRETREATS,
      ServiceNameEnum.ALLNATION,
      ServiceNameEnum.PROMEDEXPRESS,
      ServiceNameEnum.SUMEXUS,
  ...
  def get_query_request(self, selections, fragments: Dict[str, FragmentDefinitionNode] = None, finalData: list = None, lastNest: int = None, currNode: str = None, rootNode: str = None) -> list[tuple]:
    ...
    for i in range(len(selections)):
      ...
      self.get_query_request(selections=selectionsData, fragments=fragments, finalData=finalData, lastNest=lastNest + 1, currNode=currNode, rootNode=rootNode)

    return finalData
  
  def convert_to_db_cols(self, root_node: str, query_context: list[tuple] = None, info: GraphQLResolveInfo = None, exclude: list = None) -> str:
    if not query_context and not info:
      self.http_400_bad_request_response("Must provide the graphql resolver info or use the LinkRequest.get_query_request function")
    ...
    return [
      node[1]
      for node in query_context
      if node[0] == root_node and node[1] not in exclude
    ]

  def convert_to_db_cols_with_attr(self, resultObject, root_node: str, query_context: list[tuple] = None, info: GraphQLResolveInfo = None, exclude: list = None):
    return [
      getattr(resultObject, v)
      for v in self.convert_to_db_cols(info=info, root_node=root_node, query_context=query_context, exclude=exclude)
    ]
  ...
  def general_validation_process(
    ...
    # validate guest
    if not guest and (not token_decode.account_info_id or token_decode.account_info_id == 0 or token_decode.user_role == AccountRoleEnum.GUEST):
      self.http_401_unauthorized_response("Unauthorized guest access")
      
    # validate role access
    if roles and not token_decode.user_role in roles:
      self.http_401_unauthorized_response("Unauthorized role access")
      
    # validate company
    if company and (not token_decode.account_company_id or token_decode.account_company_id == 0 or token_decode.user_role in [AccountRoleEnum.GUEST, AccountRoleEnum.CUSTOMER]):
